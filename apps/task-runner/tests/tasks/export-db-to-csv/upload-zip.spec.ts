...
import { uploadZip } from '@/task-runner/tasks/export-db-to-csv/upload-zip';
...
describe('uploadZip', () => {
  ...
  it('should upload to S3 and return the download URL', async () => {
    ...
    const downloadUrl = await uploadZip(zipFileName, zipBuffer);
  ...
  it('should throw an error for unsupported storage provider', async () => {
    ...
    await expect(uploadZip(zipFileName, zipBuffer)).rejects.toThrow(
